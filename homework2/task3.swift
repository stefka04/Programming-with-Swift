...
class HStack: VisualGroup {
  ...
  func remove(child: VisualComponent) {
    ...
    while index >= 0 {
      if child.isEqualTo(other: children[index]) {
        removeChild(at: index)
        break
      }
      index += 1
...
func count(root: VisualComponent?, level: Int) -> Int {
  ...
  if let currentGroup = root as? VisualGroup {
    if level == 2 {
      return currentGroup.numChildren
    }
    var resultCount = 0 
    for child in currentGroup.children { 
      resultCount += count(root: child, level: level - 1)
    }
    return resultCount
  }
  return 0
